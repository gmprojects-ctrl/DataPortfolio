...
// Swapt column a and b in bmat
int swapcolumn(band_mat *bmat, long a, long b){
  double temp = 0;
  double current =0;
  for(int i=0; i<bmat->ncol;i++){
    temp = getv(bmat,i,a);
    current = getv(bmat,i,b);
    setv(bmat,i,a,current);
    setv(bmat,i,b,temp);
  }
  return 0;
}
...
int swapf2l2(double * array, long len){
  double temp = array[0];
...
// Folding the mat array

int matfold(band_mat* bmat){
  double* copy = (double *)malloc(sizeof(double)*bmat->ncol*bmat->ncol);
  if(copy == NULL){
    return 1;
  }
  for(long i=0; i<bmat->ncol;i++){
    for(long j =0; j<bmat->ncol;j++){
      copy[i*bmat->ncol + j] = getv(bmat,i,j);
      
    }}
  for(long i=0; i<bmat->ncol;i++){
    for(long j =0; j<bmat->ncol;j++){
      long fold_i = fold(i,bmat->ncol);
      long fold_j = fold(j,bmat->ncol);
      setv(bmat,fold_i,fold_j,copy[i*bmat->ncol + j ]);
      //printf("(%ld,%ld) |-> (%ld,%ld)\n",i,j,fold_i,fold_j);
    }
    //printf("\n");
  }
...
int arrayfold(double * arr, long N_t, long N_z){
  double present_value =0;
  double current_value =0;
  for(int i =0; i<N_t; i++){
    for(int j =0; j<N_z;j++){
      present_value = arr[indx(i,j,N_z)];
      current_value = arr[indxfold(i,j,N_t,N_z)];
      arr[indxfold(i,j,N_t,N_z)] = present_value + current_value;
...
long write_output(double time,long N_t,long N_z,double* T,char *fname){  
  ...
  for(int i = 0; i< N_t;i++){
    for(int j =0; j< N_z;j++){
      ...
      double cTemp = T[indxfold(i,j,N_t,N_z)];
...
int main(){
    ...
    if ( S == NULL || T == NULL || T_prev == NULL || coeff == NULL || foldarr == NULL ){
   ...
   for(int i=0; i < ncols; i++){
    ...
    current_value = -a*(coeff[indxmod(l_i+1,l_j,N_t,N_z)].Q11 - coeff[indxmod(l_i-1,l_j,N_t,N_z)].Q11) - b*(coeff[indxmod(l_i,l_j+1,N_t,N_z)].Q22 - coeff[indxmod(l_i,l_j-1,N_t,N_z)].Q22) - coeff[indxmod(l_i,l_j,N_t,N_z)].R - 1/time_step ; 
    past_value = getv(&bmat,fold_row,m);
    //printf("%d %ld\n",i,m);
    setv(&bmat,fold_row,m,past_value+current_value);  
    //setv(&bmat,i,m,555);
  ...
  do{
    ...
    for(int i=0; i < ncols; i++){
      ...
      // Changing the Timestep value in the array
      //e T_{i}{j} 
      m = indxfold(l_i ,l_j,N_t,N_z);
      current_value = -a*(coeff[indxmod(l_i+1,l_j,N_t,N_z)].Q11 - coeff[indxmod(l_i-1,l_j,N_t,N_z)].Q11) - b*(coeff[indxmod(l_i,l_j+1,N_t,N_z)].Q22 - coeff[indxmod(l_i,l_j-1,N_t,N_z)].Q22) - coeff[indxmod(l_i,l_j,N_t,N_z)].R - 1/time_step ; 
    ...
    while(ctime < f_t){
      arr_copy(T_prev,T,ncols);
      // Setting up our RHS b using fold arr
      for(int i=0; i<ncols; i++){
        gridp(i,N_z,&l_i,&l_j);
        long row = indxmod(l_i,l_j,N_t,N_z);
        m = indxfold(l_i ,l_j,N_t,N_z);
        foldarr[m] = -(S[row] + (1/time_step)*T_prev[m]);
        
        }
        solve_Ax_eq_b(&bmat, T, foldarr);
        ctime+=time_step;
    }
  }while (converge(T,T_prev,ncols) != 0 && I_COUNTER < I_MAX);

  write_output(f_t,N_t,N_z,T,"output.txt");
